function [Markers, VideoFrameRate, AnalogSignals, AnalogFrameRate, Event, ParameterGroup, CameraInfo, ResidualError, MissingMarker] = loadc3d(FullFileName)
% LOADC3D - Retrieves 3D coordinate/analog data from a C3D file 
%
% Input: 
% FullFileName       File name of trial
% showWaitBar        showing progress bar? (true/[false])
%
% Outputs:
...
% Ver. 2.2 Revision (Adam Zabicki, Gießen, June 2020) [get rid of "progress_bar", it slowed down performance dramatically!
%                                                      also, add new *MissingMarker" variable, indicating "Residual = -1" timepoints ]
...
MissingMarker = nan(NvideoFrames, Nmarkers);

if Scale < 0    
    % Negative scale factor: Float32-data
    for i = 1:NvideoFrames   %XXXXXXXXXXXXXX hier flaschenhals XXXXXXXXX
        % Marker and AD data
        ...
        % mark missing marker if residual == -1
        missing = squeeze(DataM(1:Nmarkers, 4)) == -1;
        MissingMarker(i, missing) = 1;
        MissingMarker(i, ~missing) = 0;
    end    
else
